...
async function startQuiz() { /* ... 原有測驗程式碼不變 ... */ }
function parseQuestions(text) { /* ... 原有 ... */ }
function renderQuestion() { /* ... 原有 ... */ }
function checkAnswer(selectedBtn, selectedLetter, correctLetter, allOptionsText) { /* ... 原有 ... */ }
function nextQuestion() { /* ... 原有 ... */ }
...
async function submitToGemini() {
    ...
    const requestData = {
        contents: [{
            parts: [
                { 
                    text: "這是一張長者口腔照片，請用溫暖、親切、鼓勵的語氣，用繁體中文詳細分析清潔狀況、有沒有問題，並給予簡單易懂的改善建議。" 
                },
                { 
                    inlineData: { 
                        mimeType: "image/jpeg", 
    ...
    try {
        ...
        resultBox.innerHTML = `
            ❌ 分析失敗<br><br>
            <small style="color:red;">錯誤：${error.message}</small><br>
            <small>請檢查網路連線，或稍後再試一次。</small>
        `;
